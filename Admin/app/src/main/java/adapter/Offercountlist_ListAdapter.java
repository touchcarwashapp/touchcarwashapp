...
import com.touchcarwashadmin.Add_Offer;
...
import com.touchcarwashadmin.Offer_Management;
...
import data.OfferCount_FeedItem;
import data.Washprice_FeedItem;

public class Offercountlist_ListAdapter extends BaseAdapter {
    ...
    Typeface face;
    public List<OfferCount_FeedItem> feedItems;
    private LayoutInflater inflater;
    public String pcatid = "";
    ProgressDialog pd;
    int pos = 0;
    public Offercountlist_ListAdapter(Activity activity2, List<OfferCount_FeedItem> feedItems2) {
        activity = activity2;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.custom_offer, null);
        }
        TextView vehiclecompany= (TextView) convertView.findViewById(R.id.vehiclecompany);
        TextView washcount=convertView.findViewById(R.id.washcount);
        ImageView delete = (ImageView) convertView.findViewById(R.id.delete);
        ImageView edit = (ImageView) convertView.findViewById(R.id.edit);
        ImageView image = (ImageView) convertView.findViewById(R.id.image);

        OfferCount_FeedItem item = (OfferCount_FeedItem) feedItems.get(position);
        ...
        washcount.setTypeface(face);

        vehiclecompany.setText(item.getCompanyname());
        washcount.setText(item.getWashcount());
        ...
        delete.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                try {
                    ...
                    pcatid = item.getSn();
                     pos = position;
                   showalert_delete("Are you sure want to delete this offer ?");
                } catch (Exception e) {
                }
            }
        });
        edit.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                OfferCount_FeedItem item = (OfferCount_FeedItem) feedItems.get(position);
                Temp.offeredit = 1;
                Temp.offer_sn=item.getSn();
                Temp.offer_companyname=item.getCompanyname();
                Temp.offer_vehicleid=item.getVehicleid();
                Temp.offer_washcount=item.getWashcount();
                Temp.offer_offercount=item.getOffercount();
                Temp.offer_imgsig=item.getImgsig();
    ...
    public class delete_product extends AsyncTask<String, Void, String> {
        ...
        public String doInBackground(String... arg0) {
            try {
                String link= Temp.weblink +"delete_offercount.php";
