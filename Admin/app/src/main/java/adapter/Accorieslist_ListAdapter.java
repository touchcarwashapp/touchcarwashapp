...
import com.touchcarwashadmin.Add_Accories;
...
import data.Accories_FeedItem;
import data.Washprice_FeedItem;

public class Accorieslist_ListAdapter extends BaseAdapter {
    ...
    Typeface face;
    public List<Accories_FeedItem> feedItems;
    private LayoutInflater inflater;
    public String pcatid = "";
    ProgressDialog pd;
    int pos = 0;
    public Accorieslist_ListAdapter(Activity activity2, List<Accories_FeedItem> feedItems2) {
        activity = activity2;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.custom_accories, null);
        }
        ...
        Accories_FeedItem item = (Accories_FeedItem) feedItems.get(position);
        ...
        Glide.with(context).load(Temp.weblink+"accoriessmall/"+item.getSn()+".jpg").apply(rep).transition(DrawableTransitionOptions.withCrossFade()).into(image);
        ...
        vehiclecompany.setText(item.getItemname());
        ...
        edit.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                ...
                Temp.accori_sn=item.getSn();
                Temp.accori_vehicleid=item.getVehicleid();
                Temp.accori_itemname=item.getItemname();
                Temp.accori_discription=item.getDiscription();
                Temp.accori_ogprice=item.getOgprice();
                Temp.accori_offerprice=item.getOfferprice();
                Temp.accori_imgsig=item.getImgsig();

                Intent i = new Intent(context, Add_Accories.class);
    ...
    public class delete_product extends AsyncTask<String, Void, String> {
        ...
        public String doInBackground(String... arg0) {
            try {
                String link= Temp.weblink +"delete_accories.php";
        ...
        public void onPostExecute(String result) {
            if (pd != null || pd.isShowing()) {
                ...
                if (result.contains("ok")) {
                    ...
                    ((Accorie_List) activity).removeitem(pos);
                    return;
                }
